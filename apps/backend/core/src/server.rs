...
impl Server {
	...
	pub async fn run(&mut self) {
		...
		handler.run(states.clone(), self.config.server_tx.take().unwrap()).await;
	}
		...
		Box::pin(async move {
			Ok({
				...
				if let Ok(state) = state {
					...
					state.create_terminal_shell(terminal_shell_builder_id, terminal_shell_id).await;
	...
	fn resize_terminal_shell(
		...
		Box::pin(async move {
			Ok({
				...
				if let Ok(state) = state {
					let mut state = state.lock().await;
					state.resize_terminal_shell(terminal_shell_id, cols, rows).await;
		...
		Box::pin(async move {
			Ok({
				...
				if let Ok(state) = state {
					...
					state.create_language_server(language_server_builder_id).await;
		...
		Box::pin(async move {
			Ok({
				...
				if let Ok(state) = state {
					...
					state.write_to_language_server(language_server_id, data).await;
