...
impl State {
	...
	pub async fn update(&mut self, new_data: StateData) {
		...
		if let Some(persistor) = &self.persistor {
			...
			if data_has_changed {
				...
				info!("Data from State by id <{}>, hasn't been modified", self.data.id);
			}
		} else {
			warn!("Persistor not found for State by id <{}>, could not save", self.data.id);
		}
	}
	...
	pub async fn create_terminal_shell(
		...
		if let Some(shell_builder) = shell_builder {
			...
			self.terminal_shells.insert(terminal_shell_id.to_string(), Arc::new(shell));
		} else {
			warn!(
	...
	pub async fn write_to_terminal_shell(&self, terminal_shell_id: String, data: String) {
		...
		if let Some(shell) = shell {
			...
			warn!("Could not write to non-existent terminal shell, id <{}>", terminal_shell_id);
		}
	}
	...
	pub async fn create_language_server(&mut self, language_server_builder_id: String) {
		let language_server_builder =
			self.language_server_builders.get(&language_server_builder_id);
		...
		if let Some(language_server_builder) = language_server_builder {
			...
			self.language_servers.insert(info.id, Arc::new(Mutex::new(language_server)));
		} else {
			warn!(
	...
	pub async fn write_to_language_server(&mut self, language_server_id: String, data: String) {
		...
		if let Some(language_server) = language_server {
			...
			warn!("Language Server by id <{}> is not running", language_server_id);
		}
	}

	/// Terminate to a Language Server instance
	pub async fn unload_language_server(&mut self, language_server_builder_id: &str) {
		self.language_server_builders.remove(language_server_builder_id);
		self.language_servers.remove(language_server_builder_id);
		self.extensions_manager
			.sender
			.send(ClientMessages::ServerMessage(ServerMessages::UnloadedLanguageServer {
