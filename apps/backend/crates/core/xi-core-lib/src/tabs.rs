	...
	pub(crate) fn client_notification(&mut self, cmd: CoreNotification) {
		...
		match cmd {
			...
			SetLanguage {
				view_id,
				language_id,
			} => self.do_set_language(view_id, language_id),
		}
	}
	...
	fn notify_client_and_update_views(&self) {
		{
			...
			self.peer
				.theme_changed(style_map.get_theme_name(), style_map.get_theme_settings());
		}
...
impl CoreState {
	...
	fn detect_whitespace(&mut self, id: ViewId, config: &Table) -> Option<Table> {
		...
		let config_delta = self
			.config_manager
			.table_for_update(ConfigDomain::SysOverride(buffer_id), changes);
		match self
			.config_manager
