...
impl HealthMonitor {
            ...
            Some(health) => {
                ...
                if health.circuit_breaker_open {
                    if let Some(opened_at) = health.circuit_breaker_opened_at {
                        ...
                        if elapsed.to_std().unwrap_or(Duration::ZERO) >= self.config.circuit_breaker_timeout {
    ...
    pub async fn perform_health_check(&self, endpoint_id: &str) -> HealthResult<HealthCheck> {
        ...
        // Update endpoint health
        {
            ...
            if let Some(endpoint_health) = endpoints.get_mut(endpoint_id) {
                ...
                return Err(crate::error::HealthError::EndpointNotRegistered(endpoint_id.to_string()));
            }
        }
    ...
    pub async fn start_monitoring(&self) -> HealthResult<()> {
        ...
        *running = true;
        ...
        // that periodically checks all registered endpoints
