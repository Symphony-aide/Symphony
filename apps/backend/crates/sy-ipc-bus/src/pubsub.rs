...
impl PubSubManager {
	/// Create a new pub/sub manager with default capacity
	///
	/// # Examples
	///
	/// ```rust
	...
	/// Create a new pub/sub manager with custom channel capacity
	///
	/// # Arguments
	///
	/// * `capacity` - Default channel capacity for new topics
	///
	/// # Examples
	///
	/// ```rust
	/// use sy_ipc_bus::PubSubManager;
	///
	/// let manager = PubSubManager::new_with_capacity(5000);
	/// ```
	#[must_use]
	pub fn new_with_capacity(capacity: usize) -> Self {
		duck!("Creating new PubSubManager with capacity: {}", capacity);
	...
	pub async fn subscribe(&self, pattern: &str) -> PubSubResult<EventReceiver> {
		...
		if Self::is_pattern(pattern) {
			...
			let topic_channel =
				topics.entry(pattern.to_string()).or_insert_with(|| TopicChannel::new(self.default_capacity));
	...
	pub async fn publish(&self, topic: &str, message: MessageEnvelope) -> PubSubResult<usize> {
		...
		if delivered_count == 0 {
			duck!("No subscribers found for topic: {}", topic);
		}
	...
	fn compile_pattern(pattern: &str) -> PubSubResult<Regex> {
		...
		if pattern.is_empty() {
			return Err(crate::error::PubSubError::InvalidPattern(
		...
		if pattern.contains('[') && !pattern.contains(']') {
			return Err(crate::error::PubSubError::InvalidPattern(
				"Unmatched bracket in pattern".to_string(),
			));
		}
