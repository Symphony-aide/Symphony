...
use sy_ipc_protocol::{MessageMetadata, MessagePriority, MessageType};
...
impl MessageEnvelopeTestFactory {
	...
	pub fn event() -> MessageEnvelope {
		let id = safe_generator().next_unique_id();
		MessageEnvelope::new(MessageType::DataAccess, format!("event-payload-{}", id).into_bytes())
	}

	/// Generate a message with specific routing key
	pub fn with_routing_key(_routing_key: &str) -> MessageEnvelope {
		let id = safe_generator().next_unique_id();
		MessageEnvelope::new(MessageType::PitOperation, format!("payload-{}", id).into_bytes())
	}
