...
use crate::factory::{
	EndpointIdTestFactory, MessageEnvelopeTestFactory, PriorityTestFactory, RoutingKeyTestFactory,
};
use std::time::Duration;
use sy_ipc_bus::{BusConfig, MessageBus};
...
mod tests {
	...
	async fn test_bus_config_default_values() {
		...
		assert_eq!(config.pubsub_channel_capacity, 1000);
		assert_eq!(config.routing_latency_threshold(), Duration::from_nanos(100_000));
		assert_eq!(config.correlation_cleanup_interval(), Duration::from_secs(30));
	}
	...
	async fn test_bus_config_custom_values() {
		...
		let custom_config = BusConfig {
			...
			correlation_timeout_secs: 60,
			health_check_interval_secs: 10,
			batch_size: 200,
			max_concurrent_deliveries: 2000,
			pubsub_channel_capacity: 2000,
			routing_latency_threshold_ns: 200_000, // 200μs
			correlation_cleanup_interval_secs: 60,
		};
