...
mod tests {
    ...
    async fn test_resolve_correlation_success() {
        ...
        manager.register_request(correlation_id.clone(), reply_to.clone()).await.unwrap();
    ...
    async fn test_pending_count_tracking() {
        ...
        manager.register_request(correlation_id1.clone(), reply_to.clone()).await.unwrap();
        assert_eq!(manager.pending_count().await, 1);
        
        // Act - register second correlation
        manager.register_request(correlation_id2.clone(), reply_to.clone()).await.unwrap();
