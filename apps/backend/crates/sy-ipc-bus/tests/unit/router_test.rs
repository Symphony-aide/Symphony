...
use crate::factory::{EndpointIdTestFactory, PriorityTestFactory, RoutingKeyTestFactory};
...
mod tests {
	...
	async fn test_find_all_routes_multiple_matches() {
		...
		router
			.register_route("user.*", endpoint1.clone(), PriorityTestFactory::high())
			.await
			.unwrap();
		router
			.register_route("*.create", endpoint2.clone(), PriorityTestFactory::medium())
			.await
			.unwrap();
	...
	async fn test_route_priority_ordering() {
		...
		router
			.register_route(pattern, low_priority_endpoint.clone(), PriorityTestFactory::low())
			.await
			.unwrap();
		router
			.register_route(pattern, high_priority_endpoint.clone(), PriorityTestFactory::high())
			.await
			.unwrap();
		...
		let route = found_route.unwrap();
		assert_eq!(
			route.endpoint_id, high_priority_endpoint,
			"Higher priority route should be selected"
		);
	}
}
