...
pub enum SerializationError {
    ...
    #[error("Performance target violated: {operation} took {duration_ms}ms, expected <{target_ms}ms")]
...
impl MessagePackSerializer {
        ...
        T: Serialize + Send + Sync,
    {
        ...
        if duration_micros > 10 && cfg!(not(debug_assertions)) {
            duck!("MessagePack serialization took {}μs (target: <10μs in release)", duration_micros);
        ...
        T: DeserializeOwned + Send + Sync,
    {
        ...
        // Allow more flexibility in debug/test builds
        if duration_micros > 10 && cfg!(not(debug_assertions)) {
            duck!("MessagePack deserialization took {}μs (target: <10μs in release)", duration_micros);
            return Err(SerializationError::PerformanceViolation {
