...
use sy_ipc_protocol::{
    MessageEnvelope, MessageType, MessageMetadata, MessagePriority, CorrelationId,
    SerializationFormat, MessageSerializer, JsonRpcMessage, JsonRpcRequest,
    MessageRegistry,
};
...
mod registry_tests {
    ...
    async fn test_registry_serialization() {
        ...
        let serialized = registry.serialize_message(&envelope, SerializationFormat::MessagePack).await.unwrap();
        let deserialized: MessageEnvelope<String> = registry.deserialize_message(&serialized, SerializationFormat::MessagePack).await.unwrap();
