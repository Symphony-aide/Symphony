...
use sy_ipc_protocol::{
	CorrelationId, MessageEnvelope, MessageMetadata, MessagePriority, MessageType,
};

// Import factory module
use factory::{
	CorrelationIdTestFactory, MessageEnvelopeTestFactory, MessageMetadataTestFactory,
	UUIDTestFactory,
};
...
mod message_envelope_tests {
	...
	#[test]
	fn test_message_envelope_direct_creation() {
		duck!("Testing MessageEnvelope direct creation");

		let envelope: MessageEnvelope<String> = MessageEnvelope::new(
