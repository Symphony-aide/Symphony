...
impl TextEditingPort for MockTextEditingAdapter {
    ...
    async fn get_buffer_content(&self, buffer_id: BufferId) -> Result<String> {
        ...
        self.buffers.lock().unwrap()
            .get(&buffer_id)
            .cloned()
            .ok_or_else(|| crate::errors::PortError::TextEditingFailed {
                message: "Buffer not found".to_string(),
            }.into())
    }
    
    async fn insert_text(&self, buffer_id: BufferId, position: Position, text: &str) -> Result<()> {
        duck!("MockTextEditingAdapter::insert_text called for {:?} at {:?}: {}", buffer_id, position, text);
    ...
    async fn delete_text(&self, buffer_id: BufferId, range: Range) -> Result<()> {
        duck!("MockTextEditingAdapter::delete_text called for {:?} range {:?}", buffer_id, range);
    ...
    async fn replace_text(&self, buffer_id: BufferId, range: Range, text: &str) -> Result<()> {
        duck!("MockTextEditingAdapter::replace_text called for {:?} range {:?}: {}", buffer_id, range, text);
    ...
    async fn save_buffer_as(&self, buffer_id: BufferId, path: PathBuf) -> Result<()> {
        duck!("MockTextEditingAdapter::save_buffer_as called for {:?} to {:?}", buffer_id, path);
    ...
    async fn create_view(&self, buffer_id: BufferId) -> Result<ViewId> {
        ...
        // Send event if sender exists
        if let Some(sender) = self.event_sender.lock().unwrap().as_ref() {
            let _ = sender.send(TextEditingEvent::ViewCreated {
                view_id,
                buffer_id,
            });
        }
    ...
    async fn set_cursor(&self, view_id: ViewId, position: Position) -> Result<()> {
        ...
        // Send event if sender exists
        if let Some(sender) = self.event_sender.lock().unwrap().as_ref() {
            let _ = sender.send(TextEditingEvent::CursorMoved {
                view_id,
                position,
            });
        }
    ...
    async fn set_selection(&self, view_id: ViewId, range: Range) -> Result<()> {
        ...
        // Send event if sender exists
        if let Some(sender) = self.event_sender.lock().unwrap().as_ref() {
            let _ = sender.send(TextEditingEvent::SelectionChanged {
                view_id,
                range,
            });
        }
...
impl PitPort for MockPitAdapter {
    ...
    async fn get_model_status(&self, handle: ModelHandle) -> Result<ModelStatus> {
        ...
        self.models.lock().unwrap()
            .get(&handle)
            .map(|(_, status)| *status)
            .ok_or_else(|| crate::errors::PortError::PitOperationFailed {
    ...
    async fn create_workflow(&self, definition: WorkflowDefinition) -> Result<WorkflowId> {
        ...
        self.workflows.lock().unwrap().insert(workflow_id, (definition, WorkflowStatus::Pending));
        Ok(workflow_id)
    }
    
    async fn execute_workflow(&self, workflow_id: WorkflowId, _context: ExecutionContext) -> Result<WorkflowResult> {
    ...
    async fn submit_decision(&self, context: DecisionContext) -> Result<Decision> {
        duck!("MockPitAdapter::submit_decision called for context type: {}", context.context_type);
...
impl ExtensionPort for MockExtensionAdapter {
    async fn load_extension(&self, manifest: ExtensionManifest) -> Result<ExtensionId> {
        ...
        self.extensions.lock().unwrap().insert(extension_id, (manifest, ExtensionStatus::Loaded));
        Ok(extension_id)
    }
    ...
    async fn send_message(&self, extension_id: ExtensionId, _message: ExtensionMessage) -> Result<ExtensionResponse> {
    ...
    async fn broadcast_message(&self, _message: ExtensionMessage) -> Result<Vec<ExtensionResponse>> {
        duck!("MockExtensionAdapter::broadcast_message called");
        
        let extensions = self.extensions.lock().unwrap();
        let responses = extensions.keys()
...
impl ConductorPort for MockConductorAdapter {
    ...
    async fn submit_decision(&self, context: DecisionContext) -> Result<Decision> {
        duck!("MockConductorAdapter::submit_decision called for context type: {}", context.context_type);
        ...
        metrics.average_confidence = (metrics.average_confidence * (metrics.total_decisions - 1) as f64 + decision.confidence) / metrics.total_decisions as f64;
    ...
    async fn get_decision_history(&self, limit: usize) -> Result<Vec<DecisionRecord>> {
        ...
        let records: Vec<DecisionRecord> = decisions.iter()
            .take(limit)
    ...
    async fn submit_feedback(&self, decision_id: DecisionId, feedback: Feedback) -> Result<()> {
        duck!("MockConductorAdapter::submit_feedback called for decision {:?} with score: {}", decision_id, feedback.score);
...
impl DataAccessPort for MockDataAccessAdapter {
    ...
    async fn get_workflow(&self, workflow_id: WorkflowId) -> Result<WorkflowSpec> {
        ...
        self.workflows.lock().unwrap()
            .get(&workflow_id)
            .cloned()
            .ok_or_else(|| crate::errors::PortError::DataAccessFailed {
                operation: "get_workflow".to_string(),
                message: "Workflow not found".to_string(),
            }.into())
    }
    
    async fn update_workflow(&self, workflow_id: WorkflowId, updates: WorkflowUpdates) -> Result<()> {
    ...
    async fn authorize_action(&self, user_id: UserId, action: Action) -> Result<bool> {
        duck!("MockDataAccessAdapter::authorize_action called for user {:?} action: {}", user_id, action.action_type);
    ...
    async fn update_extension_status(&self, extension_id: ExtensionId, status: ExtensionStatus) -> Result<()> {
        duck!("MockDataAccessAdapter::update_extension_status called for: {:?} status: {:?}", extension_id, status);
