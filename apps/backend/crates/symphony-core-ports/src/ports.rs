...
pub trait PitPort: Send + Sync {
    ...
    async fn execute_workflow(&self, workflow_id: WorkflowId, context: ExecutionContext) -> Result<WorkflowResult>;
...
pub trait ExtensionPort: Send + Sync {
    ...
    async fn send_message(&self, extension_id: ExtensionId, message: ExtensionMessage) -> Result<ExtensionResponse>;
