...
mod mock_determinism_tests {
    ...
    async fn test_mock_extension_determinism() {
        ...
        for i in 0..3 {
            ...
            manifest_copy.name = format!("test-extension-{}", i);
    ...
    async fn test_mock_data_access_determinism() {
        ...
        for i in 0..3 {
            let mut workflow_copy = workflow.clone();
            workflow_copy.name = format!("test-workflow-{}", i);
...
mod mock_performance_tests {
    ...
    async fn test_mock_text_editing_performance() {
        ...
        // Mock operations should be very fast (<1ms target)
    ...
    async fn test_mock_conductor_performance() {
        ...
        let start = Instant::now();
        let _ = mock.start_conductor().await.unwrap();
        let duration = start.elapsed();
    ...
    async fn test_mock_data_access_performance() {
        ...
        let start = Instant::now();
        let _ = mock.pre_validate_workflow(&workflow).await.unwrap();
        let duration = start.elapsed();
...
mod mock_error_injection_tests {
    ...
    async fn test_mock_pit_error_injection() {
        ...
        let spec = ModelSpec {
            ...
            model_type: ModelType::Language,
            resource_requirements: ResourceRequirements {
                memory_mb: 999999, // Unrealistic memory requirement
                cpu_cores: 2.0,
                gpu_memory_mb: None,
                network_access: false,
            },
    ...
    async fn test_mock_extension_error_injection() {
        ...
        let manifest = ExtensionManifest {
            ...
            permissions: vec![],
            dependencies: vec![],
            entry_point: "nonexistent.py".to_string(),
        };
