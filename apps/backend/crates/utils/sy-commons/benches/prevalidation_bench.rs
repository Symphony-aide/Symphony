...
fn benchmark_non_empty_rule(c: &mut Criterion) {
	...
	c.bench_function("non_empty_rule", |b| b.iter(|| rule.validate(black_box(&value))));
}
...
fn benchmark_min_length_rule(c: &mut Criterion) {
	...
	c.bench_function("min_length_rule", |b| b.iter(|| rule.validate(black_box(&value))));
}
...
fn benchmark_max_length_rule(c: &mut Criterion) {
	...
	c.bench_function("max_length_rule", |b| b.iter(|| rule.validate(black_box(&value))));
}
...
fn benchmark_composite_rule(c: &mut Criterion) {
	...
	c.bench_function("composite_rule", |b| b.iter(|| rule.validate(black_box(&value))));
}
