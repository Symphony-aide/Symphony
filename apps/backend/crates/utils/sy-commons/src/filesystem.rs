...
pub async fn read_file<P: AsRef<Path>>(path: P) -> Result<String, SymphonyError> {
	...
	tokio::fs::read_to_string(path).await.map_err(|e| SymphonyError::Io {
		source: e,
...
pub async fn write_file<P: AsRef<Path>>(path: P, content: &str) -> Result<(), SymphonyError> {
	...
	tokio::fs::write(&temp_path, content).await.map_err(|e| SymphonyError::Io {
		source: e,
		context: Some(format!("Failed to write temp file: {}", temp_path.display())),
	})?;

	tokio::fs::rename(&temp_path, path).await.map_err(|e| SymphonyError::Io {
		source: e,
...
pub async fn create_dir_all<P: AsRef<Path>>(path: P) -> Result<(), SymphonyError> {
	...
	tokio::fs::create_dir_all(path).await.map_err(|e| SymphonyError::Io {
		source: e,
