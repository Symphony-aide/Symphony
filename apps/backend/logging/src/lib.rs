...
fn setup_async_file_logging(file_path: &str, env_filter: EnvFilter) -> Result<WorkerGuard, String> {
	...
	let directory = path.parent().ok_or_else(|| "Invalid file path".to_string())?;
	let filename = path
		.file_name()
		.and_then(|n| n.to_str())
	...
	tracing_subscriber::registry()
		.with(env_filter)
		.with(fmt::layer().json().with_current_span(false).with_writer(non_blocking))
		.init();
...
fn setup_console_logging(env_filter: EnvFilter) {
	fmt().with_env_filter(env_filter).pretty().with_target(false).init();
}
