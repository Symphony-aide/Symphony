...
export class HTTPClient extends Emittery implements Client {
	 ...
	 * @JsonRpcMethod
	 */
	public write_file_by_path(path: string, content: string, filesystem_name: string): Promise<CoreResponse<never>> {
	 ...
	 * @JsonRpcMethod
	 */
	public get_ext_info_by_id(extension_id: string): Promise<CoreResponse<ManifestInfo>> {
		return this.rpc.call("get_ext_info_by_id", [extension_id, this.config.state_id, this.config.token]);
	}
	...
	public get_ext_list(): Promise<CoreResponse<string[]>> {
		return this.rpc.call("get_ext_list", [this.config.state_id, this.config.token]);
	}
	...
	public get_all_language_server_builders(): Promise<CoreResponse<LanguageServer[]>> {
		return this.rpc.call("get_all_language_server_builders", [this.config.state_id, this.config.token]);
	}
	...
	public get_terminal_shell_builders(): Promise<CoreResponse<TerminalShellBuilderInfo[]>> {
		return this.rpc.call("get_terminal_shell_builders", [this.config.state_id, this.config.token]);
	}

	public write_to_terminal_shell(terminal_shell_id: string, data: string): Promise<CoreResponse<never>> {
	...
	public close_terminal_shell(terminal_shell_id: string): Promise<CoreResponse<never>> {
		return this.rpc.call("close_terminal_shell", [this.config.state_id, this.config.token, terminal_shell_id]);
	}
	...
	public resize_terminal_shell(terminal_shell_id: string, cols: number, rows: number): Promise<CoreResponse<never>> {
	...
	public create_language_server(language_server_builder_id: string): Promise<CoreResponse<never>> {
	...
	public write_to_language_server(language_server_id: string, data: string): Promise<CoreResponse<never>> {
